...
```{r}
# bad, stemmed, stop words removed
...
out <- data.frame(polar_out, topic1, topic2)
out$polar2 <- out$polar_out^2
out$polar3 <- out$polar_out^3

reg1 <- lm(topic1 ~ poly(polar_out, degree=3), data=out)
reg2 <- lm(topic2 ~ poly(polar_out, degree=3), data=out)
library(stargazer)

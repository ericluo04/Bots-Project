        ...
        if (len(cnext)>0 and keepgoing and accessible):
                ...
                while True:
                        ...
                        try :
                                ...
                                print("The Twitter servers are up, but overloaded with requests. Will try again in 20 seconds.")
                                time.sleep(20)                                     
                        jsonResponse = response.read()  
                        res=jsonResponse.decode().split('\n')
                        ufriends+=[i.split('/follow/')[1].split('"')[0] for i in res if '/i/guest/follow/' in i ]
...
writeCSVFile([i[0] for i in INACCESSIBLE], 'failed_to_query_'+str(date)+'_'+str(batch_number)+'.csv')
